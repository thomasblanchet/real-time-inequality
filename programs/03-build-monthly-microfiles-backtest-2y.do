...
foreach t of numlist $date_begin / $date_end {
    ...
    quietly {
        ...
        save "$work/03-build-monthly-microfiles-backtest-2y/microfiles/dina-monthly-`year'm`month'.dta", replace
        ...
        save "$work/03-build-monthly-microfiles-backtest-2y/rank-flemp-gpinter.dta", replace
        ...
        use "$work/03-build-monthly-microfiles-backtest-2y/microfiles/dina-monthly-`year'm`month'.dta", clear
        ...
        save "$work/03-build-monthly-microfiles-backtest-2y/rank-uiinc-gpinter.dta", replace
        
        // Export tabulation to interpolate
        use if year == max(1976, `year' - 2) using "$work/02-prepare-ui/dina-ui-dist.dta", clear
        keep p avg_uiinc
        ...
        merge 1:1 id pid using "$work/03-build-monthly-microfiles-backtest-2y/flemp-gpinter.dta", nogenerate assert(master match)
        merge 1:1 id pid using "$work/03-build-monthly-microfiles-backtest-2y/uiinc-gpinter.dta", nogenerate assert(master match)
