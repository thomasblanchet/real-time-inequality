...
foreach t of numlist $date_begin / $date_end {
    ...
    quietly {
        ...
        if (`year' == 1976) {
            ...
            use if inlist(year, `year' - 1) using "$work/02-prepare-dina/dina-simplified-normalized.dta", clear
        }
        ...
        save "$work/03-build-monthly-microfiles-backtest-1y/microfiles/dina-monthly-`year'm`month'.dta", replace
        ...
        save "$work/03-build-monthly-microfiles-backtest-1y/rank-flemp-gpinter.dta", replace
        ...
        keep p flemp_qcew
        save "$work/03-build-monthly-microfiles-backtest-1y/tabul-flemp-gpinter.dta", replace
        
        // Interpolate with gpinter
        rsource using "$programs/03-interpolate-qcew.R", roptions(`" --vanilla --args "$work/03-build-monthly-microfiles-backtest-1y" "')
        
        // ------------------------------------------------------------------ //
        // Same for UI benefits
        // ------------------------------------------------------------------ //
        
        use "$work/03-build-monthly-microfiles-backtest-1y/microfiles/dina-monthly-`year'm`month'.dta", clear
        ...
        save "$work/03-build-monthly-microfiles-backtest-1y/rank-uiinc-gpinter.dta", replace
        
        // Export tabulation to interpolate
        use if year == max(1976, `year' - 1) using "$work/02-prepare-ui/dina-ui-dist.dta", clear
        keep p avg_uiinc
        save "$work/03-build-monthly-microfiles-backtest-1y/tabul-uiinc-gpinter.dta", replace
        
        // Interpolate with gpinter
        rsource using "$programs/03-interpolate-uiinc.R", roptions(`" --vanilla --args "$work/03-build-monthly-microfiles-backtest-1y" "')
        ...
        merge 1:1 id pid using "$work/03-build-monthly-microfiles-backtest-1y/flemp-gpinter.dta", nogenerate assert(master match)
        merge 1:1 id pid using "$work/03-build-monthly-microfiles-backtest-1y/uiinc-gpinter.dta", nogenerate assert(master match)
